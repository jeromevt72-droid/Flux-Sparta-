...
async function leaderboard(request, env) {
  // Minimal, additive CORS fix (GET-only, read-only, no auth/cookies) --
  // required for the production Gateway's LIVE GLOBAL GRID, which is
  // deployed on its own separate domain per instructions and cannot read
  // this endpoint cross-origin without it. No other endpoint, no other
  // behavior, and no existing caller's behavior is changed by this.
  const corsHeaders = { "Access-Control-Allow-Origin": "*" };
  if (!env.LEADERBOARD) return json({ error: "Leaderboard storage is not configured yet (missing LEADERBOARD KV binding)" }, 500, corsHeaders);
  const url = new URL(request.url);
  ...
  return json({ top: top.slice(0, limit), countries, leadingCountry: countries[0] || null }, 200, { "Cache-Control": "max-age=15", ...corsHeaders });
}
