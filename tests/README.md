...
| `test-*.mjs`, `regression-suite.py`, `fuzz.mjs`, `run-all-tests.mjs` | The release gate (37 suites) |
...
`node run-all-tests.mjs` — must print **RELEASE GATE PASSED: all 37 suites passed**.
`test-d35-browser.mjs`, `test-rc287-browser.mjs`, `test-level-banner-browser.mjs` and `test-backdrop-bleed-browser.mjs` are extra (they need a Chromium browser via Playwright) and are not in the gate.
