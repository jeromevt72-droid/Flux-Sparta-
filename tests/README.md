...
`test-d35-browser.mjs`, `test-rc287-browser.mjs`, `test-level-banner-browser.mjs`, `test-backdrop-bleed-browser.mjs` and `test-debug-readout-browser.mjs` are extra (they need a Chromium browser via Playwright) and are not in the gate.
